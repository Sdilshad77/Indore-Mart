    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(getCart.fulfilled, (state, action) => {
                state.cartLoading = false
                state.cartSuccess = true
                state.cartItems = action.payload  // null = not logged in, that's fine
                state.cartError = false
            })
...
export const getCart = createAsyncThunk("CART/FETCH", async (_, thunkAPI) => {
    ...
    if (!user) return null  // not logged in — silently skip, no error
    ...
    try {
        ...
        // Auth error (expired/invalid token) — clear bad session silently
        if (status === 400 || status === 401 || status === 403) {
            localStorage.removeItem('user')
            return null  // return null cart, don't show error
        }
        return thunkAPI.rejectWithValue(message)
    }
